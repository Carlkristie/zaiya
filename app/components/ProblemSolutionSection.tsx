...
export default function ProblemSolutionSection() {
  ...
  return (
    <section className="relative bg-white text-slate-900 py-24 lg:py-32">
      <div className="max-w-7xl mx-auto px-8 lg:px-16">
        ...
        >
          <h2 className="text-4xl md:text-6xl lg:text-7xl font-light leading-tight font-[family-name:var(--font-space-grotesk)] text-slate-800 mb-6">
            Problems we solve
          </h2>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 lg:gap-8 items-start">
          {problems.map((problem, index) => (
            <motion.div
              ...
              className="bg-white border border-slate-200 rounded-2xl overflow-visible hover:shadow-xl transition-shadow duration-300 relative"
            >
              <div className="p-8">
                <div className="mb-6">
                  <div className="text-slate-400 line-through text-lg mb-3">
                    {problem.pain}
                  </div>
                  <div className="flex items-start gap-3">
                    <span className="text-2xl">→</span>
                    <h3 className="text-2xl md:text-3xl font-medium text-slate-800 leading-tight font-[family-name:var(--font-space-grotesk)]">
                      {problem.outcome}
                    </h3>
                  </div>
                </div>
                ...
                <button
                  ...
                  className="w-full text-left text-sm font-medium text-slate-600 hover:text-slate-900 transition-colors flex items-center justify-between group lg:pointer-events-none"
                >
                  <span>What you get</span>
                  <motion.span
                    animate={{ rotate: isCardExpanded(problem.id) ? 180 : 0 }}
                    transition={{ duration: 0.3 }}
                    className="text-xl group-hover:translate-y-0.5 transition-transform"
                  >
                    ↓
                  </motion.span>
                </button>
              ...
              >
                <div className="px-8 pb-8 space-y-4 pt-2 border-t border-slate-200 mx-8">
                  {problem.steps.map((step, stepIndex) => (
                    <div key={stepIndex} className="flex gap-3">
                      <span className="flex-shrink-0 w-6 h-6 rounded-full bg-slate-800 text-white text-xs flex items-center justify-center font-medium mt-0.5">
                        {stepIndex + 1}
                      </span>
